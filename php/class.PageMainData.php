		...
		foreach ($_collection as $order) {
			...
			<li id="order-'. $_orderID.'">
				<a href="#">'.
					...
					'<span class="form_1_order_ul_li_span">'. ($_orderAcknowledged ? '<img class="infotip" title="'. $this->__t->__('Printed'). ' '. $_orderAckTime. '" width="16" src="images/print-icon.png">&nbsp;<em>' : ''). $_orderID. ' - '. $order->getBillingAddress()->getName(). ' - '. $order->getCustomer_email(). ' - '. $_orderStatus. ' - '. $order->getCreatedAt(). ($_orderAcknowledged ? '</em>' : ''). '</span>
				</a>
			</li>';
		}


		$this->set('orderulhtml','<p>'. $_count. ' '. $this->__t->__('Order/s'). ' (<span id="ordertimerange" class="edit_order_time_range">'. $this->get('ordertimerange'). '</span> '. $this->__t->__('Day/s'). ')</p><ul id="form_1_order_ul" class="order_ul">'. ($_count==0 ? '<li>'. $this->__t->__('No Orders Found'). '</li>' : '<li><input type="checkbox" id="form_1_order_ul_select_allorders" name="form_1_order_ul_select_allorders" value="null"><span class="textSmall">'. $this->__t->__('Select All'). '</span>  <input type="checkbox" id="form_1_order_ul_select_allorders_notack" name="form_1_order_ul_select_allorders_notack" value="null"><span class="textSmall">'. $this->__t->__('Select Not Printed'). '</span></li>'. $_ULHtml) . '</ul>');
